...
// Bar pill for the Environment Canada radar loop. Mirrors the contract the
// first-party weather widget implements: the bar tracks THIS item as the
// panel identity (popout coordinator, open-panel dot, panel switching), so
...
BarWidget {
  ...
  // Unlike weather there is nothing to hide behind — the glyph is static, so
  // the pill is always present once the widget is in the layout.
  visible: true
